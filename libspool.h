 ...
 * set to 1
 */
#define set_all(size) (1 << size) - 1

/*
 * Returns 1 if a given slot is
 * available
 */
#define pool_check(ind, p) (p->avail & (1<<ind))
 ...
 * occupied slots
 */
#define pool_count(p) p->size - bits_set(p->avail)
 ...
 */
int pool_push(void *data, pool *p){
  int out;
